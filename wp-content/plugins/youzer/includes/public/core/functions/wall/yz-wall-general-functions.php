...
/**
 * Activity - Giphy Action
 */
function yz_activity_action_wall_giphy( $action, $activity ) {

	// Prepare Action.
	$action = sprintf( __( '%s added a new Gif', 'youzer' ), bp_core_get_userlink( $activity->user_id ) );

	return apply_filters( 'yz_activity_new_giphy_action', $action, $activity );
...
function yz_get_wall_post_content( $content, $activity ) {
    ...
    if ( 'joined_group' == $activity->type ) {
    	...
    	$content = yz_get_wall_new_post( $activity->item_id, $activity->secondary_item_id );
 ...
 * 	Wall Embed Post
 */
function yz_get_wall_new_post( $blog_id = false, $post_id = false ) {
