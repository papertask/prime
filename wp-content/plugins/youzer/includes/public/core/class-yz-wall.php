...
class Youzer_Wall {
	...
	function wall_post_attachments( $activity_id = null ) {
		...
		switch ( $post_type ) {
			...
			case 'cover':
				...
				break;
			case 'giphy':
				$this->get_wall_post_giphy( $activity_id );
				break;
		}
	...
	function get_wall_post_video( $attachments ) {
		...
		$video_content = '<video width="100%" controls preload="metadata"><source src="' . $video_url . '" type="video/mp4">' . __( 'Your browser does not support the video tag.', 'youzer' ) . '</video>';
