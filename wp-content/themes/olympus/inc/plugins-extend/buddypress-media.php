...
function _filter_olympus_rtmedia_media_thumb( $src, $id, $type ) {
	...
	if ( $type === 'music' ) {
		...
		return get_template_directory_uri() . '/images/media-audio.png';
	} else if ( $type === 'video' ) {
		if ( $thumbnail ) {
			return $thumbnail;
		}

		return get_template_directory_uri() . '/images/media-video.png';
	} else if ( $type === 'album' ) {
	...
	<li class="photo-album-item-wrap rtmedia-list-item col-3-width">
		<div class="photo-album-item create-album">
			...
			<div class="content">
				...
				<span class="title h5">Create an Album</span>
...
function olympus_register_rtmedia_metabox() {
	add_meta_box( 'olympus_rtmedia_metabox_id', esc_html__( 'Olympus rtMedia', 'olympus' ), 'olympus_render_rtmedia_metabox', 'nav-menus', 'side', 'default' );
}
