...
if ( !function_exists( 'olympus_font_url' ) ) {
	...
	function olympus_font_url() {
		...
		$default_theme_font = 'Roboto:300,400,500,700:latin';
		...
		$font_families[]	 = $default_theme_font;
		$custom_google_fonts = implode( '|', $font_families );
		if ( $default_theme_font == $custom_google_fonts ) {
			$font_url = get_template_directory_uri() . '/css/roboto-font.css';
		} else {
			$font_url	 = esc_url( add_query_arg( 'family', rawurlencode( $custom_google_fonts ), '//fonts.googleapis.com/css' ) );
...
if ( !function_exists( 'olympus_sidebar_conf' ) ) {
	...
	function olympus_sidebar_conf( $sb_wide = false ) {
		$sidebar	 = array( 'col-md-12', 'col-sm-12' );
...
if ( !function_exists( 'olympus_generate_font_styles' ) ) {
	...
	function olympus_generate_font_styles( $tag ) {
		...
		if ( 'nav' === $tag ) {
			$font_css = '.left-menu .left-menu-title,'
	...
	function olympus_bp_menu() {
		...
		if ( class_exists( 'Youzer' ) ) {
			...
			$links[ 'change-password' ] = array(
				'icon'	 => 'olymp-share-post-icon',
				'href'	 => yz_get_settings_url( 'change-password', $user_id ),
				'title'	 => __( 'Change Password', 'olympus' )
			);
		}
...
if ( !function_exists( 'olympus_highlight_searched' ) ) {
	...
	function olympus_highlight_searched( $searched = '', $text = '' ) {
		...
		return preg_replace( '/' . $searched . '(?!([^<]+)?>)/i', '<span class="bg-primary highlight-searched">$0</span>', $text );
	}
...
function olympus_is_top_menu_panel_visible() {
	$olympus = Olympus_Options::get_instance();
	$visible = $olympus->get_option_final( 'top-menu-panel-options/show', 'no' );
